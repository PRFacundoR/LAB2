<?php
	...
	require_once 'conexion.php';//paso 1
?>
    <section class="row justify-content-center">
        <!-- programar debajo de aquí, debe mostrar los datos de la tabla -->
<?php	
	if(!empty($_COOKIE['mi-tipo'])) {
		$categor = $_COOKIE['mi-tipo'];
		$conexion=conectar();
		
		if($conexion){
			$consulta='SELECT * FROM noticias WHERE categoria=?';
			
			$sentencia=mysqli_prepare($conexion, $consulta);
			mysqli_stmt_bind_param($sentencia,'s',$categor);
